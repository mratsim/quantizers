...
def test_sharegpt_format_loading():
    ...
    config = CalibrationSetConfig(
        ...
        datasets=[
            DatasetEntryConfig(
                dataset=str(Path(__file__).parent / "test_datasets" / "sharegpt_format"),
                split="train",
...
def test_prompt_answer_format_loading():
    ...
    config = CalibrationSetConfig(
        ...
        datasets=[
            DatasetEntryConfig(
                dataset=str(Path(__file__).parent / "test_datasets" / "prompt_answer_format"),
                split="train",
...
def test_chat_completion_format_loading():
    ...
    config = CalibrationSetConfig(
        ...
        datasets=[
            DatasetEntryConfig(
                dataset=str(Path(__file__).parent / "test_datasets" / "chat_completion_format"),
                split="train",
                columns=["messages"],
...
def test_multiple_dataset_loading():
    ...
    for i, row in enumerate(raw_dataset):
        ...
        if i < 2:
            ...
            assert len(messages) == 1, f"Row {i} raw text format should have exactly one message"
            # Check for assistant message
            has_assistant = any(msg.get("role") == "assistant" for msg in messages)
            assert has_assistant, f"Row {i} missing assistant message"
...
def test_error_handling_missing_dataset():
    ...
    config = CalibrationSetConfig(
        ...
        datasets=[
            DatasetEntryConfig(
                dataset=str(Path(__file__).parent / "test_datasets" / "nonexistent_format"),
                split="train",
                columns=["messages"],
                formatter="chat_completion",
                num_samples=10
...
def test_arbitrary_column_names():
    ...
    config = CalibrationSetConfig(
        ...
        datasets=[
            DatasetEntryConfig(
                dataset=str(Path(__file__).parent / "test_datasets" / "custom_columns_format"),
                split="train",
        ...
        config = CalibrationSetConfig(
            ...
            datasets=[
                DatasetEntryConfig(
                    dataset=str(Path(__file__).parent / "test_datasets" / "sharegpt_format"),
                    split="train",
                    columns=["conversations", "extra_column"],
                    formatter="sharegpt",
                    num_samples=10
                )
            ]
...
def test_missing_columns_keyerror():
    ...
    try:
        ...
        print(f"✅ ShareGPT formatter correctly raised KeyError for missing column: {str(e)}")
    except Exception as e:
    ...
    try:
        ...
        print(f"✅ Prompt-answer formatter correctly raised KeyError for missing column: {str(e)}")
    except Exception as e:
    ...
    try:
        ...
        print(f"✅ Raw text formatter correctly raised KeyError for missing column: {str(e)}")
    except Exception as e:
