...
def test_chat_completion_formatter_field_validation():
    ...
    nested_result = DatasetFmt.chat_completion(["messages_col"], nested_data["messages_col"]["messages"])
...
def test_raw_text_formatter_with_arbitrary_columns():
    ...
    test_cases = [
        ...
        (["my_custom_field"], {"my_custom_field": "Fifth document with numbers: 1234567890"})
    ]
        ...
        mock_dataset = datasets.Dataset.from_dict({
            "formatted": [
                ...
                [
                    ...
                    {"role": "assistant", "content": "Photosynthesis is the process by which plants convert light energy..."}
                ],
                [
                    {"role": "user", "content": "How does the internet work?"},
                    {"role": "assistant", "content": "The internet is a global system of interconnected computer networks..."}
                ]
            ]
        })
...
def test_calibration_set_with_arbitrary_columns():
    ...
    config = CalibrationSetConfig(
        ...
        datasets=[
            DatasetEntryConfig(
                dataset="test/dataset",
                split="train",
                columns=["user_query", "model_response"],  # Arbitrary column names
                formatter="prompt_answer",
                num_samples=2
            )
        ]
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create mock dataset with arbitrary column names
        mock_dataset = datasets.Dataset.from_dict({
            "user_query": [
        ...
        # Manually apply formatting to simulate what _consolidate_datasets does
        def format_row(row):
            # Extract data using arbitrary column names
            data = dict(zip(config.datasets[0].columns, [row[col] for col in config.datasets[0].columns]))
            # Apply formatter with the columns parameter
            return {"formatted": DatasetFmt.prompt_answer(config.datasets[0].columns, data)}
        
        # Apply formatting using map
        formatted_dataset = mock_dataset.map(format_row, remove_columns=mock_dataset.column_names)
        
        # Verify our formatting worked correctly
        assert len(formatted_dataset) == 2
        assert "formatted" in formatted_dataset.column_names
        
        # Check that the formatting produced the expected results
        result_0 = formatted_dataset[0]["formatted"]
        result_1 = formatted_dataset[1]["formatted"]
        
        assert len(result_0) == 2  # Should have both user and assistant messages
        assert len(result_1) == 2
        
        assert result_0[0]["role"] == "user"
        assert result_0[0]["content"] == "What is machine learning?"
        assert result_0[1]["role"] == "assistant"
        assert "Machine learning" in result_0[1]["content"]
        
        assert result_1[0]["role"] == "user"
        assert result_1[0]["content"] == "Explain photosynthesis"
        assert result_1[1]["role"] == "assistant"
        assert "Photosynthesis" in result_1[1]["content"]
        
        print("✅ CalibrationSet works correctly with arbitrary column names")
